            ...
            <div className='w-full flex flex-wrap'>
                <div className='w-full md:w-3/12'>
                    <div className='w-full p-4 bg-white shadow-md rounded-md text-[#2a2b4c]'>
                        <div className='flex-col  justify-center items-center pb-11'>
                            {
                                userInfo?.image ?
                                    <label htmlFor='img' className='h-[300px] w-[300px] relative cursor-pointer overflow-hidden'>
                                        <img src={userInfo.image} alt="" className=' rounded-3xl w-full h-full' />
                                        {loader && <div className='bg-white  h-full w-full opacity-80 left-0 top-0 z-20 flex justify-center items-center absolute'>
                                            <span>
                                                <HashLoader color='#739072'></HashLoader>
                                            </span>
                                        </div>}
                                    </label> :
                                    <label className='flex rounded-md justify-center items-center flex-col h-[250px] w-[270px] cursor-pointer border border-dashed border-black hover:border-green-500 relative' htmlFor="img">
                                        <span><BsImage /></span>
                    ...
                    <div className='w-full pl-0 md:pl-7 mt-6 md:mt-0  text-[#2a2b4c]'>
                        <div className='bg-white shadow-md rounded-md p-4' >
                            <div className=' py-2 px-0 md:px-5'>
                                <div className='flex justify-between text-sm flex-col gap-3 pb-5 relative border-b border-[#3a4d39] '>
                                    ...
                                    <div className='flex gap-4'>
                                        <span>Status: </span>
                                        {userInfo?.status === 'active' ? <span className='text-green-500'>{userInfo.status}</span> : <span className='text-red-500'>{userInfo.status}</span>}
                                    </div>
                                    <div className='flex gap-4'>
                                        ...
                                        <p >
                                            {
                                                userInfo?.payment === 'active'
                                                    ? <span className='text-green-500 font-normal'>{userInfo.payment} </span>
                                                    : <button onClick={() => dispatch(create_stripe_connect_account())} className=' text-green-500  cursor-pointer  font-normal'>click active </button>
                                            }
                                        </p>
                                    </div>
                                </div>
                            ...
                            <div className='py-2 px-0 md:px-5 '>
                                {
                                    userInfo?.shopInfo ?
                                        <div className='flex justify-between text-sm flex-col gap-3 pb-5 relative '>
                                            <div className='flex items-center'>
                                                <div className='text-xl font-semibold flex items-center gap-2'>
                                        ...
                                        </div> : <form onSubmit={add} className='flex flex-col gap-2'>
                                            <div className='flex flex-col gap-1 w-full'>
                                                <label htmlFor="Shop">Shop Name</label>
                                                <input onChange={inputHandle} value={state.shopName} type="text" className='px-3 py-1 outline-none border bg-transparent border-slate-700 rounded-md  focus:border-green-500 overflow-hidden w-full' placeholder='Shop name' id='Shop' name='shopName' />
                                            </div>
                                            <div className='flex flex-col gap-1 w-full'>
                                                <label htmlFor="Division">Division</label>
                                                <input onChange={inputHandle} value={state.Division} type="text" className='px-3 py-1 outline-none border bg-transparent border-slate-700 rounded-md  focus:border-green-500 overflow-hidden w-full' placeholder='Division' id='Division' name='Division' />
                                            </div>
                                            <div className='flex flex-col gap-1 w-full'>
                                                <label htmlFor="District">District</label>
                                                <input onChange={inputHandle} value={state.District} type="text" className='px-3 py-1 outline-none border bg-transparent border-slate-700 rounded-md  focus:border-green-500 overflow-hidden w-full' placeholder='District' id='District' name='District' />
                                            </div>
                                            <div className='flex flex-col gap-1 w-full'>
                                                <label htmlFor="Sub">Sub District</label>
                                                <input onChange={inputHandle} value={state.subDistrict} type="text" className='px-3 py-1 outline-none border bg-transparent border-slate-700 rounded-md  focus:border-green-500 overflow-hidden w-full' placeholder='Sub district' id='sub' name='subDistrict' />
                                            </div>
                                ...
                                {
                                    show === true && <form onSubmit={add} className='flex flex-col gap-2'>
                                        <div className='flex flex-col gap-1 w-full'>
                                            <label htmlFor="Shop">Shop Name</label>
                                            <input onChange={inputHandle} value={state.shopName} type="text" className='px-3 py-1 outline-none border bg-transparent border-slate-700 rounded-md focus:border-green-500 overflow-hidden w-full' placeholder='Shop name' id='Shop' name='shopName' />
                                        </div>
