    ...
    return (
        <div className='px-2 lg:px-7 pt-5'>
            <div className='flex lg:hidden justify-between items-center mb-5 p-4 bg-white shadow-md rounded-md'>
                ...
                <button className='bg-red-500 shadow-lg px-4 py-2 cursor-pointer text-white rounded-md text-sm'>Add Category</button>
            </div>
            <div className='flex flex-wrap w-full'>
                                ...
                                <tbody>
                                    {
                                        coupons.map((el, index) => <tr key={index}>
                                            ...
                                            <td scope='row' className='py-3
                                             px-4 font-medium whitespace-nowrap'><span>{moment(el.expire).format('DD/MM/YYYY h:mm:ss A')}</span></td>
                                            <td scope='row' className='py-3
                                             px-4 font-medium whitespace-nowrap'><span>{Date.now() < new Date(el.expire).getTime() ? 'active' : 'deactive'}</span></td>
                                            <td scope='row' className='py-3
                                             px-4 font-medium whitespace-nowrap'>
                                            ...
                                            <td scope='row' className='py-3
                                             px-4 font-medium'>
                                                <div className='flex justify-start  items-center gap-4'>
                                                    ...
                                                    <button onClick={() => delete_Coupon(el._id)}><FaTrash color='#FF494C' size={17} /></button>
                                                </div>
                                            </td>
                                        </tr>)
                                    }
                ...
                <div className={`w-[320px] lg:w-5/12 translate-x-100 lg:relative lg:right-0 fixed z-10 top-0 transition-all duration-500 `}>
                    <div className='w-full pl-5'>
                        <div className=' bg-white shadow-md rounded-md h-screen lg:h-auto px-3 py-2 lg:rounded-md text-[#2B2A4C]'>
                            <div className='flex justify-between items-center mb-4'>
                                ...
                                {/* <div className='block lg:hidden cursor-pointer'><IoMdClose size={25} className='text-[#2B2A4C] hover:text-[#FF494C]' /></div> */}
                            </div>
                            <form onSubmit={add} >
                                <div className='flex flex-col w-full gap-2 mb-3'>
                                    <label htmlFor="name">Coupon Name</label>
                                    <input onChange={(e) => setCouponName(e.target.value)} value={couponName} required className='px-3 py-2 outline-none border bg-transparent border-slate-700 rounded-md text-[#2B2A4C] focus:border-[#3a4d39] overflow-hidden' type="text" id='name' name='coupon_name' placeholder='coupon name' />
                                </div>
                                <div className='flex flex-col w-full gap-2 mb-3'>
                                    <label htmlFor="time">Coupon time</label>
                                    <input onChange={(e) => setCouponTime(e.target.value)} value={couponTime} required className='px-3 py-2 outline-none border bg-transparent border-slate-700 rounded-md text-[#2B2A4C] focus:border-[#3a4d39] overflow-hidden' type="datetime-local" id='time' name='coupon_time' placeholder='coupon time' />
                                </div>
                                <div className='flex flex-col w-full gap-2 mb-3'>
                                    <label htmlFor="percent">% Coupon</label>
                                    <input onChange={(e) => setCouponPercent(e.target.value)} value={couponPercent} required className='px-3 py-2 outline-none border bg-transparent border-slate-700 rounded-md text-[#2B2A4C] focus:border-[#3a4d39] overflow-hidden' type="number" min={0} max={100} id='percent' name='coupon_percent' placeholder='coupon percent' />
                                </div>
                                <div>
